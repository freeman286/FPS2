...
public class PlayerEquipment : NetworkBehaviour
{
    ...
    private float timeSinceEquipmentUsed = 100f;
    ...
    void Update()
    {
        ...
        if (isLocalPlayer && Input.GetButtonDown("Equipment") && timeSinceEquipmentUsed > equipment.cooldown) {
            if (equipment is Grenade)
            {
                CmdGrenadeThrow(equipmentSpawnPoint.position, Quaternion.LookRotation(equipmentSpawnPoint.forward, Random.insideUnitCircle), transform.name, ((Grenade)equipment).throwPower);
    ...
    {
        GameObject _grenade = (GameObject)Instantiate(((Grenade)equipment).prefab, _pos, _rot);
